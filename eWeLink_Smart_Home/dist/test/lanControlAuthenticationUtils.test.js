"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var lanControlAuthenticationUtils_1 = __importDefault(require("../utils/lanControlAuthenticationUtils"));
function test() {
    var iv = 'OTU0MTc4NTg3MjQzMjU4Mw';
    iv = lanControlAuthenticationUtils_1.default.decryptionBase64(iv);
    var key = '8a031b29-8ee1-4bd4-b5ce-143738445bfd';
    var data = '';
    console.log('iv ', iv);
    console.log('key ', key);
    // console.log('salt ', salt);
    console.log('data ', data);
    // console.log('----------------------------------------------------------------');
    // let baseCiper = LanControlAuthenticationUtils.encryptionData({iv, salt, key, data});
    // console.log('加密:', baseCiper);
    console.log('----------------------------------------------------------------');
    var decryptedData = lanControlAuthenticationUtils_1.default.decryptionData({ data: data, iv: iv, key: key });
    console.log('解密:', decryptedData);
}
test();
// let _base64Str = LanControlAuthenticationUtils.encryptionBase64('1234567890123456');
// let tmp = `12345${Date.now()}67890`.slice(0, 16);
// console.log(tmp.length);
// const str = LanControlAuthenticationUtils.encryptionBase64(tmp);
// console.log(str);
// console.log('base64加密', _base64Str);
// console.log('base64解密', LanControlAuthenticationUtils.decryptionBase64('RDuhJiY1n0J6W4tZfL4jVA=='));
